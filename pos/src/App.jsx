...
grocery.map(product => {
  product.categories.map(categories => categoriesSet.add(categories));
})
...
function App() {
  ...
  let catProducts = (categories == "all") ?
    grocery : (categories.indexOf("search") != -1) ?
      grocery.filter((item) => {
        return (item.categories[0].toLowerCase().indexOf(searchItem.toLowerCase()) != -1) || (item.product_name.toLowerCase().indexOf(searchItem.toLowerCase()) != -1)
      }) :
      grocery.filter((item) => item.categories[0] == categories);
                ...
                <div className=" mx-auto mb-3 mt-3" style={{ width: "100%" }}>
                  <input
                    onChange={(event) => {
                      ...
                      setCategories("search" + searchItem)
                    }}
